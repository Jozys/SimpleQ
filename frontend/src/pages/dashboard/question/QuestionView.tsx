...
import {
  QuestionPaginationNext,
  QuestionPaginationPrev,
} from "../../../components/question/QuestionPagination";
...
import AdjustUserContent, {
  SortByDef,
  SortDirectionDef,
} from "../../../components/AdjustUserContent";
...
export default function QuestionView(props: Props) {
  ...
  const [question, setQuestion] = React.useState<QuestionDef | undefined>(
    undefined
  );
  ...
  const [activeAuthorId, setActiveAuthorId] = React.useState<
    string | undefined
  >(undefined);
  ...
  useEffect(() => {
    global.axios
      .get("question/" + encodeURIComponent(id ?? ""), {
    ...
    global.axios
      .get(
        "question/" +
          ...
          "&enableAI=" +
          encodeURIComponent(enableAI) +
          "&limit=" +
          encodeURIComponent(ANSWERS_PAGE_SIZE) +
          "&offset=" +
          encodeURIComponent(answersPage * ANSWERS_PAGE_SIZE)
      )
      .then((res) => {
  ...
  const toggleFavorite = async () => {
    ...
    if (question.isFavorite)
      await global.axios
        .delete("favourites/" + id, { withCredentials: true })
        .then((_) => setQuestion({ ...question, ...{ isFavorite: false } }))
        .catch((err) => axiosError(err, alert));
    else
      await global.axios
        .post("favourites/" + id, {}, { withCredentials: true })
  ...
  return (
    ...
    >
      <ActiveProfileModal
        activeProfileId={activeAuthorId}
        isOwner={false}
        closeModal={() => setActiveAuthorId(undefined)}
      />

      <QuestionTitle
        authenticated={auth.getToken() != null}
        question={question}
      ...
      {(question?.answers ?? 1) > 0 && (
        <>
          ...
          <QuestionPaginationPrev
            setAnswersPage={() => setAnswersPage(answersPage - 1)}
      ...
      {(question?.answers ?? 1) > 0 && (
        <>
          ...
          <QuestionPaginationNext
            setAnswersPage={() => setAnswersPage(answersPage + 1)}
            enabled={
              (answersPage + 1) * ANSWERS_PAGE_SIZE < (question?.answers ?? 0)
            }
            inlineElement={
              <p className={"caption"} style={{ alignSelf: "flex-start" }}>
                Showing answers {answersPage * ANSWERS_PAGE_SIZE + 1} to{" "}
      ...
      <QuestionAnswerEditor
        ...
        onSubmit={async (content) => {
          ...
          await global.axios
            .post(
              "question/" + encodeURIComponent(id ?? "") + "/answer",
              { content },
              { withCredentials: true }
            )
            .then((_) => {
