...
export default function App() {
  ...
  useEffect(() => {
    const updateThemeVariables = () => {
      ...
      variables.forEach((variable) => {
        let value =
          theme === "dark" || (theme === "system" && themePreference === "dark")
  ...
  useEffect(() => {
    const updateThemePreference = (matches: boolean) => {
      if (theme === "system") setTheme(matches ? "dark" : "light");
    };

    window
      .matchMedia("(prefers-color-scheme: dark)")
      .addEventListener("change", ({ matches }) =>
        updateThemePreference(matches)
      );
  ...
  const [profile, setProfile] = React.useState<ProfileDef | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    console.log("effect");
    global.axios
      .get<any>("profile", { withCredentials: true })
      .then((res) => {
        setProfile({
          id: res.data.userId ?? "",
      ...
      <Routes>
        ...
        >
          ...
          <Route
            ...
            element={
              <Suspense>
                <QuestionView setActiveQuestion={setActiveQuestion} />
              </Suspense>
            }
          />
          <Route
