...
export default function QuestionAnswer(props: Props) {
  ...
  return (
    <Section className={"transparent"} style={{ alignItems: "stretch" }}>
      <div
        style={{
          display: "flex",
      ...
      >
        {props.answer ? (
          props.answer.author.type === "ai" ? (
            <i
              className={"fi fi-rr-brain avatar"}
              style={{
                fontSize: "1.4em",
      ...
      <div style={{ flex: 1 }}>
        ...
        >
            ...
            >
              {props.answer?.author ? (
                <span
                  ...
                  onClick={() =>
                    props.answer?.author &&
                    props.setActiveProfile &&
                    props.setActiveProfile(props.answer.author.id)
                  }
                  onKeyUp={(e) =>
                    e.key === "Enter" &&
                    props.answer?.author &&
            ...
            <span className={"caption"}>
              {props.answer ? (
                <span style={{ display: "inline-flex" }}>
                  ...
                  />
                  {t("dashboard.questionView.answer.creationDate", {
                    answerCreationDate: formatDate(props.answer.created),
                  })}
                </span>
              ) : (
                <Skeleton width={80} />
          ...
          {props.answer && (
            <ButtonGroup style={{ marginBottom: "calc(var(--spacing) / 2)" }}>
              <Button
                slim
                iconLeft={"fi fi-rr-social-network"}
                buttonStyle={
                  props.answer?.opinion === "dislike" ? "primary" : "glass"
                }
                onClick={async () =>
                  props.postVote &&
                  (await props.postVote(
                    props.answer?.opinion === "like" ? "none" : "like"
                  ))
                }
              ...
              <Button
                ...
                onClick={async () =>
                  props.postVote &&
                  (await props.postVote(
                    props.answer?.opinion === "dislike" ? "none" : "dislike"
                  ))
                }
                disabled={!props.authenticated}
              >
        ...
        <div style={{ display: "flex" }}>
          <div
            ...
            style={{
              ...
              background:
                "var(--background-color-glass" +
                (props.answer?.author?.type === "ai" ? "-simp" : "") +
                ")",
            }}
          >
            {props.answer?.content ? (
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: sanitizeHtml(props.answer.content),
                }}
              />
            ) : (
              <div>
